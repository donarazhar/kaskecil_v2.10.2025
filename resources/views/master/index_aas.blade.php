...
<style>
    /* ── Page vars (re-use dari sidebar layout) ── */
    :root {
        --blue:     #0053C5;
        --blue-dk:  #003d91;
        --blue-50:  #eff6ff;
    ...
    .info-meta { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; margin-top: 2px; }
...
<div class="ph fade-up">
    <div class="ph-left">
        <h2><i class="fas fa-database"></i> Master Akun AAS</h2>
        <p>Kelola data Akun Anggaran Aktivitas Satuan (AAS)</p>
    </div>
    @if(Auth::user()->level == 'admin')
    <button class="btn-blue" id="btnTambahAas">
...
<div class="panel fade-up d1">
    <div class="panel-header">
        <h6 class="panel-title"><i class="fas fa-list"></i> Daftar Akun AAS</h6>
        <span style="font-size:12px; color:var(--gray-400);">Total: {{ $aas->total() }} data</span>
    </div>
            ...
            <div class="search-row">
                <div class="search-input-wrap">
                    ...
                    <input type="text" class="search-input" name="nama_akunaas"
                        placeholder="Cari nama akun AAS..."
                ...
                <a href="/master/aas" class="btn-outline">
    ...
    <div class="tbl-wrap">
        <table class="data-table">
            ...
            <tbody>
                @forelse($aas as $d)
                <tr>
                    <td style="color:var(--gray-400); font-size:12px;">{{ $loop->iteration + $aas->firstItem() - 1 }}</td>
                    <td>
                        <div class="info-stack">
                            <span class="badge badge-blue">{{ $d->kode_aas }}</span>
                            <span class="info-name">{{ $d->nama_aas }}</span>
                        </div>
                    </td>
                    <td>
                        @if($d->kategori == 'pembentukan')
                            <span class="badge badge-blue">Pembentukan</span>
                        @elseif($d->kategori == 'pengisian')
                            <span class="badge badge-green">Pengisian</span>
                        @elseif($d->kategori == 'pengeluaran')
                            <span class="badge badge-amber">Pengeluaran</span>
                        @else
                    ...
                    <td>
                        @if($d->status == 'k')
                            <span class="badge badge-green">Kredit</span>
                        @elseif($d->status == 'd')
                    ...
                    <td>
                        <div class="act-group" style="justify-content:center">
                            ...
                            <form action="/master/aas/{{ $d->id }}/deleteaas" method="post" style="margin:0">
                                @csrf
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title-text">
                    <i class="fas fa-plus-circle"></i> Tambah Akun AAS
                </h5>
            ...
            <div class="modal-body">
                <form action="/master/storeaas" id="frmaas" method="POST">
                    ...
                    <div class="form-group">
                        <label class="form-lbl" for="kode_aas">Kode AAS <span style="color:red">*</span></label>
                        <input type="text" name="kode_aas" id="kode_aas" class="form-inp"
                            placeholder="Masukkan kode AAS (angka)" maxlength="10">
                    </div>
                    <div class="form-group">
                        <label class="form-lbl" for="nama_aas">Nama Akun AAS <span style="color:red">*</span></label>
                        <input type="text" name="nama_aas" id="nama_aas" class="form-inp"
                            placeholder="Masukkan nama akun AAS">
                    </div>
                    <div class="form-group">
                        <label class="form-lbl" for="status">Status Akun <span style="color:red">*</span></label>
                        <select name="status" id="status" class="form-sel">
                    ...
                    <div class="form-group">
                        <label class="form-lbl" for="kategori">Kategori <span style="color:red">*</span></label>
                        <select name="kategori" id="kategori" class="form-sel">
...
<div class="modal fade" id="modal-editAas" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title-text">
                    <i class="fas fa-pencil-alt"></i> Edit Akun AAS
                </h5>
                <button type="button" class="btn-close-custom" data-bs-dismiss="modal">
                    <i class="fas fa-times"></i>
                </button>
...
$(function () {
    ...
    // Simpan data — validasi dulu
    $("#btnSimpanData").click(function (e) {
        e.preventDefault();
        var kode    = $("#kode_aas").val().trim();
        var nama    = $("#nama_aas").val().trim();
        var status  = $("#status").val();
        var kategori= $("#kategori").val();

        var checks = [
            { val: kode,     msg: 'Kode AAS harus diisi',          el: '#kode_aas' },
            { val: nama,     msg: 'Nama Akun AAS harus diisi',      el: '#nama_aas' },
            { val: status,   msg: 'Status Akun harus dipilih',      el: '#status'   },
            { val: kategori, msg: 'Kategori Akun harus dipilih',    el: '#kategori' },
        ];
