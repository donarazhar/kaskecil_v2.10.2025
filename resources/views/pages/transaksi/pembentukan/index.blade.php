...
@section('header-title', 'Pembentukan Kas')
@section('content')
<style>
...
@media(max-width:640px){.ph{flex-direction:column;align-items:flex-start;}.data-table{font-size:12px;}.data-table thead th,.data-table tbody td{padding:9px 10px;}}
...
<div class="ph fade-up">
    <div class="ph-left">
        <h2><i class="fas fa-wallet"></i> Pembentukan Kas Kecil</h2>
        <p>Kelola transaksi pembentukan kas kecil</p>
    </div>
    @if(Auth::user()->level == 'admin')
    <button class="btn-blue" id="btnTambahPembentukan"><i class="fas fa-plus"></i> Tambah Data</button>
    @endif
</div>
...
<div class="panel fade-up">
    <div class="panel-header">
        <h6 class="panel-title"><i class="fas fa-list"></i> Data Pembentukan Kas Kecil</h6>
        <span style="font-size:12px;color:var(--gray-400);">Total: {{ count($pembentukan) }} data</span>
    </div>
    <div class="tbl-wrap">
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td style="color:var(--gray-400);font-size:11px;">{{ $loop->iteration }}</td>
                    ...
                    <td>
                        <div class="act-group">
                            ...
                            <form action="/transaksi/hapuspembentukan/{{ $d->id }}" method="post" style="margin:0">
                                @csrf
                ...
                @empty
                <tr><td colspan="7"><div class="empty-state"><i class="fas fa-inbox"></i><p>Belum ada data pembentukan kas kecil</p></div></td></tr>
                @endforelse
            </tbody>
            @if(count($pembentukan) > 0)
            <tfoot>
                <tr>
                    <th colspan="4" style="font-size:14px;text-transform:uppercase;">Total Pembentukan Kas</th>
                    <th colspan="3" style="text-align:right;font-size:16px;font-weight:800;color:var(--blue-dk);">
...
{{-- Modal Tambah --}}
<div class="modal fade" id="modal-frmpembentukan" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title-text"><i class="fas fa-plus-circle"></i> Tambah Pembentukan</h5>
                ...
                <form action="{{ route('transaksi.store') }}" method="post" id="frmpembentukan">
                    ...
                    <div class="form-group">
                        ...
                        <select name="kode_matanggaran" id="kode_matanggaran" class="form-sel-m">
                            ...
                            @if($m->status == 'k' && $m->kategori == 'pembentukan')
                    ...
                    <input type="hidden" name="kategori" value="pembentukan">
                    <div class="form-group">
...
{{-- Modal Edit --}}
<div class="modal fade" id="modal-editpembentukan" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title-text"><i class="fas fa-pencil-alt"></i> Edit Pembentukan</h5>
                <button type="button" class="btn-close-x" data-bs-dismiss="modal"><i class="fas fa-times"></i></button>
            </div>
...
$(function () {
    ...
    $("#btnTambahPembentukan").click(function () { $("#modal-frmpembentukan").modal("show"); });

    $("#frmpembentukan").submit(function (e) {
        var checks = [
    ...
    $(".edit").click(function () {
        ...
        $.ajax({ type:'POST', url:'/transaksi/pembentukan/edit', data:{ _token:"{{ csrf_token() }}", id:id }, success:function(r){ $('#loadeditform').html(r); } });
    });
